...
class OrderController extends StateNotifier<bool> {
  ...
  try {
    ...
    switch (response.statusCode) {
      case 200:
        ...
        // Cập nhật danh sách pending orders
        if (status == 'Pending') {
  ...
  Future<Order?> searchOrderById(String bookingId) async {
    try {
      ...
      final searchId = int.tryParse(bookingId);
  ...
  Future<void> saveRecentSearch(String bookingId) async {
    try {
      final recentSearches = await ref.read(hiveStoreService).getRecentSearches() ?? [];
  ...
  try {
    ...
    try {
      final cachedCounts = await ref.read(hiveStoreService).getOrderStatuses();
      final counts = {
